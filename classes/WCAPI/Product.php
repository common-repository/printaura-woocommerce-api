...
class Product extends Base{   
  ...
  */
  ...
  public function connectToCategory($category) {
    ...
    if($count!=0){
    $product->UpdateTaxonomy($wpdb->term_taxonomy,array('count'=>$count+1),array(
        'term_taxonomy_id'=>$category->taxonomy_id,
    ));
  }else{
       $product->UpdateTaxonomy($wpdb->term_taxonomy,array('count'=>1),array(
        'term_taxonomy_id'=>$category->taxonomy_id,
    ));
  }
  }
  ...
  public function disconnectFromCategory($category) {
    ...
    $terms=get_term_by('term_taxonomy_id',$category->taxonomy_id,'product_tag');
    $count=$terms->count;
    if($count!=0 && $terms->taxonomy=='product_tag'){
    $product->UpdateTaxonomy($wpdb->term_taxonomy,array('count'=>$count-1),array(
        'term_taxonomy_id'=>$category->taxonomy_id,
    ));
  }
