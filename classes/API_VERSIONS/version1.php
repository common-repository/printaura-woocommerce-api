        ...
        */
        ...
        if (empty($ids) && empty($skus)) {
            if ($parent_ids) {
                ...
                $ids = get_posts($args);
                $posts = $ids; // there was a heretical goto here, it's gone now.
    ...
    public function get_supported_attributes($params)
    {
        ...
        foreach ($models as $m) {
            if ($m == 'Product') {
                ...
                $model = new API\Image();
            } elseif ($m == 'FeaturedImage') {
                $model = new API\Image();
            } elseif ($m == 'Review') {
                $model = new API\Review();
            } elseif ($m == 'OrderTaxItem') {
                $model = new API\OrderTaxItem();
            } elseif ($m == 'OrderCouponItem') {
                $model = new API\OrderCouponItem();
            } elseif ($m == 'ProductAttribute') {
    ...
    */
    ...
    public function get_orders_from_trash($params)
    {
        ...
        if (empty($ids)) {
            ...
            //there was a goto here, it's gone now.
            // $models = API\Order::all("*")->per($posts_per_page)->page($paged)->fetch();
            // foreach ( $models as $model ) {
    ...
    public function update_orderitem_tracking($params)
    {
        ...
        if ($total_shipped_item > 0) {
            ...
            } elseif ($total_shipped_item == $total_items) {
                $order->updateStatus('shipped');
    ...
    public function set_coupons($params)
    {
